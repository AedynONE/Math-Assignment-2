...
PhysicsBody::PhysicsBody(int entity, BodyType bodyType, b2Body* body, std::vector<b2Vec2> points, vec2 centerOffset, bool sensor, EntityCategories category, int collidesWith, float friction, float density)
{
	...
	float lX = 999.f; // Most Left
	float rX = -999.f; // Most Right
	float bY = 999.f; // Bottom
	float tY = -999.f; // Top 
	...
	b2Vec2 polyPoints[8]; // Max size as 8

	if (points.size() < 9) // converts the array into a vector
	{
...
void PhysicsBody::ScaleBody(float scale, int fixture)
{
	...
	else
	{
		...
		{
			//Create normalized direction, don't change these
			b2Vec2 vert = bodyShape->m_vertices[i];
			lX = std::min(lX, vert.x); // Left most
			rX = std::max(rX, vert.x); // Right most
			bY = std::min(bY, vert.y); // Bottom most
			tY = std::max(tY, vert.y); // Top most

			b2Vec2 dir = (vert - center); // Direction vector
			dir.Normalize();

			//Moves the vert out by a scaled direction vector
			bodyShape->m_vertices[i] += scale * dir; // Does not support more than 8 vertices 
		}
